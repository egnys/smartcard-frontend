...
const EditForm = ({edit, setEdit, updateInfo, id, word, translation}) => {
    ...
    const onSubmit = async (event) =>{
        ...
        try {
            const fields = {word: wordCard, translation: translationCard}
            const {data} = await axios.patch(`/cards/${id}`, fields)
            handleClick()
            updateInfo()
        } catch (err){
            console.warn(err)
        ...
        <div
            ...
            <form>
                <div>
                    ...
                    <input type="text" name="text" id="text" onChange={e => setWord(e.target.value)} value={wordCard}
                ...
                <div className='mt-3'>
                    ...
                    <input type="text" name="text" id="text" onChange={e => setTranslate(e.target.value)} value={translationCard}
                           className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                           required
                    />
                </div>
